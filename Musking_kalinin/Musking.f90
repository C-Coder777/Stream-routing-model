...
!--
Subroutine Run_model(point, d, log_lik, Q_out, Q)
Use Model_data2
Use Model_data3
!--
implicit none
...
real*8  s, qrs0, qrg0, qri0, area, lat, lon, riverlen, temp, gridid
...
real*8   nash_day, nash_month, bias, log_lik ! Relative runoff error  
...
! river flow routing
  ...
  open (unit=26, file='baseflow.txt', recl=100, blank='null', position='rewind', pad='yes')
  do j = 1, n
...
qobsmonth(1)=qobs(1)
qcalmonth(1)=qj(1)
write (5,'(7a10)') 'Year','Month','Prec','Evap','Runoff','Q_obs','Q_cal'
j=1
do i=2,nt
...
call nash_bias(j,qobsmonth,qcalmonth,nash_month,bias)
  write (131,'(a110,F10.3)') 'Nash-Sutcliffe model efficiency coefficient for daily streamflow simulation:  ', Nash_day
  write (132,'(a110,F10.3)') 'Nash-Sutcliffe model efficiency coefficient for monthly streamflow simulation:  ', Nash_month
  write (133,'(a110,F10.3)') 'Relative error between Qcal and Qobs:  ', bias        
